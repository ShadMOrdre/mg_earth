...
local max, min, sin, random = math.max, math.min, math.sin, math.random
...
function mg_earth.diasq.defaultf(map, x, y, d, h)
	return h + (random()-0.5)*d
	-- return h + (sin(random()) - 0.5) * (x+y+d)
	-- return h + (16-0.5)*d
	-- return h + (0.57721-0.5)*d
	-- return h + (sin(os.clock())-0.5)*d
end

-- Create a heightmap using the specified height function (or default)
-- map[x][y] where x from 0 to map.w and y from 0 to map.h
function mg_earth.diasq.create(width, height, f)
    f = f and f or mg_earth.diasq.defaultf
    -- make heightmap
